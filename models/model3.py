...
class ImageDecoder(nn.Module):
    def __init__(self, input_size, output_channels ):
        ...
        self.upsample = nn.Sequential(
            nn.ConvTranspose2d(32, 16, 3, 2, 0, 0),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 8, 3, 2, 1, 1),
            nn.ReLU(),
            nn.ConvTranspose2d(8, 4, 3, 2, 1, 1),
            nn.ReLU(),
            nn.Conv2d(4, output_channels, kernel_size=1)
        )
...
class SequenceNUCNet3(nn.Module):
    def __init__(self, img_size):
        super(SequenceNUCNet3, self).__init__()
        self.encoder = CNN_Encoder()
        feature_size = 12 * 12 * 64
        self.decoder = LSTM_Processor(num_features=feature_size, hidden_size=512, num_layers=2)
        self.decoder_A = ImageDecoder(input_size=512, output_channels = 1)
        self.decoder_B = ImageDecoder(input_size=512, output_channels = 1)
